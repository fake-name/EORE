...
void Si570::initialize()
{
	...
	i2c_write(this->i2c_address, 135,0x01);
	delay_ms(20);
...
bool Si570::read_si570(){
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (i2c_read(this->i2c_address, 7, &(dco_reg[7]), 6) == 6)
		{
			return true;
		}
...
void Si570::write_si570()
{
	...
	// Freeze DCO
	idco = i2c_read(this->i2c_address, 137);
	i2c_write(this->i2c_address, 137, idco | 0x10 );

	i2c_write(this->i2c_address, 7, &dco_reg[7], 6);

	// Unfreeze DCO
	i2c_write(this->i2c_address, 137, idco & 0xEF);

	// Set new freq
	i2c_write(this->i2c_address, 135,0x40);
}
...
void Si570::qwrite_si570()
{
	...
	idco = i2c_read(this->i2c_address, 135);
	i2c_write(this->i2c_address, 135, idco | 0x20);

	// Write RFREQ registers
	i2c_write(this->i2c_address, 7, &dco_reg[7], 6);

	// Unfreeze the M Control Word
	i2c_write(this->i2c_address, 135, idco &  0xdf);
}
