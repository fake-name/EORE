...
void debugUnique(const __FlashStringHelper *fmt, ... ){
  char buf[128]; // resulting string limited to 128 chars
  va_list args;
  va_start (args, fmt);
#ifdef __AVR__
...
Si570::Si570(uint8_t si570_address, uint32_t calibration_frequency)
{
  ...
  DEBUG(F("Si570 init, calibration frequency = %lu"), calibration_frequency);
  Wire.begin();
  ...
  DEBUG(F("freq_xtal = %04lu"), freq_xtal);
}
...
void Si570::debugSi570()
{
  ...
  DEBUG(F("Status: %i"), status);

  for (int i = 7; i < 15; i++) {
    DEBUG(F("Register[%i] = %02x"), i, dco_reg[i]);
  }
...
int Si570::i2c_write(uint8_t reg_address, uint8_t *data, uint8_t length)
{
  ...
  if (error != 0)
  {
    DEBUG(F("Error writing %i bytes to register %i: %i"), length, reg_address, error);
    return -1;
  }
  return length;
}
...
int Si570::i2c_read(uint8_t reg_address, uint8_t *output, uint8_t length) {
  ...
  if (error != 0)
  {
    DEBUG(F("Error reading %i bytes from register %i."), reg_address);
    DEBUG(F(" endTransmission() returned %i"), error);
    return 0;
  }
  ...
  if (len != length)
  {
    DEBUG(F("Requested %i bytes and only got %i bytes"), length, len);
  }
