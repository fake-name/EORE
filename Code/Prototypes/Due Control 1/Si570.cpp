...
Si570::Si570(uint8_t si570_address, uint32_t calibration_frequency) {
	...
	delay(20);
	if (read_si570()) {
		Serial.println("Successfully initialized Si570");
		...
		Serial.println("Unable to properly initialize Si570");
		status = SI570_ERROR;
...
void Si570::debugSi570() {
	...
	debug("Reference Frequency (double): %d\n", getRfReqDouble());
}
...
int Si570::i2c_read(uint8_t reg_address, uint8_t *output, uint8_t length) {
	...
	int error = Wire.endTransmission();
	if (error != 0) {
		debug("Error reading %i bytes from register %i. endTransmission() returned %i\ni", length, reg_address, error);
		return 0;
	}
